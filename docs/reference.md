...
| `scripts-extra` | Before `</body>`          | `404.html` (`not_found.js`), `posts/single.html` (KaTeX JS when `math: true`), `_default/protected-document.html` (`protected-document.js`) |
...
| --------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `header.html`             | `baseof.html`                                           | `<title>`, description, OG tags, Twitter Card, canonical, robots, RSS link; `<link rel="preload">` for LCP avatar image; `preconnect` for OpenAlex + Last.fm (and GoatCounter in prod); `dns-prefetch` for AniList, Unsplash, TMDB, doi.org (Trakt dns-prefetch disabled 2026-08 alongside the panel) |
...
All files in `static/js/`, loaded as `type="module"` in `partials/scripts.html`. GoatCounter analytics script loads only in production (`{{ if not hugo.IsServer }}`). `scripts.html` loads `config.js` first — bundled under a per-language name (`js/config.js` for English, `js/config.mm.js` for Burmese) so its baked-in `{{ i18n }}` values match the active language — then `nav_bubble.js`, whose menu depends on `.category-header` elements being present in the DOM.
