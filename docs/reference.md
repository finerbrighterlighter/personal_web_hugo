...
- [htunteza.com — Technical Reference](#htuntezacom--technical-reference)
  ...
  - [8. Python Script — `convert_fonts.py`](#8-python-script--convert_fontspy)
    - [`subset_fonts.sh` — Roboto Mono subsetting](#subset_fontssh--roboto-mono-subsetting)
...
hugo_console/
├── assets/
│   ├── css/                   terminal-0.7.4.min.css + console.css → one concatenated, minified, fingerprinted bundle
│   ├── images/playlists/      Music-panel covers (processed to 64/128px WebP by partials/playlist-cover.html)
│   └── js/
...
├── static/
│   ├── js/                    ES6 modules (type="module") + theme-init.js (classic, blocking, in <head>)
│   ├── vendor/katex/0.16.22/  Self-hosted KaTeX: katex.min.css/js, contrib/auto-render.min.js, fonts/*.woff2
...
| `screenLimit`      | int    | `10`                             | Posters shown in the `screen.sh` strip (`panel-simkl.html`, build-time Simkl fetch); template default also `10` |
...
[[headers]]
  ...
  [headers.values]
    ...
    Content-Security-Policy = "default-src 'self'; script-src 'self' https://gc.zgo.at https://cdn.jsdelivr.net/npm/mermaid@11/; style-src 'self' 'unsafe-inline'; font-src 'self'; img-src 'self' data: https:; media-src 'self'; connect-src 'self' https://api.github.com https://api.openalex.org https://api.unsplash.com https://doi.org https://api.crossref.org https://data.crossref.org https://api.datacite.org https://data.datacite.org https://data.medra.org https://graphql.anilist.co https://ws.audioscrobbler.com https://gc.zgo.at https://stats.htunteza.com; frame-src 'self' blob:; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'self'; manifest-src 'self'"
```

Hugo version is pinned here. To upgrade: change `HUGO_VERSION` and test locally with that version first.

**Security headers:** HSTS covers the apex and all subdomains (`stats.htunteza.com` is already HTTPS-only). No `preload` token — submission to hstspreload.org is deliberately avoided.

**CSP is enforced (since 2026-09-08)** and `script-src` has **no `'unsafe-inline'`**. Everything that used to be an inline `<script>` is now an external file or an inert data blob:

| Was inline | Now |
| --- | --- |
| Theme boot script in `theme-data.html` | `static/js/theme-init.js` (classic blocking script; params via `data-ttl` / `data-lang`) |
| Burmese font-mode check in `baseof.html` | folded into `theme-init.js` (`data-lang="mm"` gate) |
| `window.__researchTags` / `__worksCount` / `__githubProjects` | `<script type="application/json" id="research-tags-data">` / `id="github-projects-data">` parsed by `research.js` / `github.js` |
| TOC level switch in `panel-post-toc.html` | `static/js/post_toc.js` (module) |
| KaTeX `onload="renderMathInElement(…)"` | `static/js/katex-init.js` (deferred, after the self-hosted KaTeX files) |
| Mermaid `<script type="module">` | `static/js/mermaid-init.js` |
| `window.goatcounter = { allow_local: false }` | `data-goatcounter-settings='{"allow_local": false}'` attribute on the count.js tag |

JSON-LD blocks (`type="application/ld+json"`) and the `theme-data` JSON blob are not executed, so they need no allowance. Directive notes: `style-src` keeps `'unsafe-inline'` (Chroma and KaTeX emit inline `style` attributes, Mermaid injects `<style>` into its SVG); `connect-src` lists the doi.org content-negotiation redirect targets (`api.crossref.org` etc.) because CSP checks every hop of a redirect; `frame-src blob:` is for the protected-document PDF iframe; `img-src https:` stays broad because Last.fm/AniList/Unsplash covers come from their own CDNs; Mermaid is allowed only under the `https://cdn.jsdelivr.net/npm/mermaid@11/` path (KaTeX is self-hosted under `static/vendor/katex/`).

**Adding a new inline script will break under this policy.** Put logic in `static/js/`, pass parameters through `data-*` attributes or a `type="application/json"` blob, and add any new fetch origin to `connect-src`. Local `hugo server` sends no headers, so test with `node scripts/csp_check.js /path/to/built/public` — it serves the build with the header from `netlify.toml`, drives the key pages (home incl. theme/CVD/duck clicks, `/mm/`, works, a work single incl. CITE → doi.org → Crossref, a math post, a mermaid post, blood, a protected document, about) with puppeteer, and exits non-zero on any `Refused to …` console message. Needs `pa11y` or `puppeteer` installed globally and chromium at `/usr/bin/chromium` (`CHROME_PATH` to override).
...
| `HUGO_SIMKL_CLIENT_ID` | `panel-simkl.html` (build time only, never shipped to the browser) | Simkl app client id (simkl.com/settings/developer). Sent as `?client_id=` on every Simkl call |
| `HUGO_SIMKL_TOKEN`     | `panel-simkl.html` (build time only, never shipped to the browser) | Long-lived Simkl PIN-flow bearer token for the site owner's library. Without both Simkl vars the `screen.sh` strip renders `$ api is not aping 🐒` and the build `warnf`s — never fatal |
| `HUGO_LASTFM_KEY`   | `lastFM.js`   | Last.fm API key                  |
| `HUGO_GITHUB_TOKEN` | `panel-github.html` (build time only, never shipped to the browser) | Optional GitHub fine-grained PAT (public repos, read-only, no extra permissions). Raises the build-side API limit from 60/h per IP to 5000/h; without it the build-time fetch still works on a quiet IP and falls back to `warnf` + client-side fetch on failure |
...
| `nav_bubble_label` | "Jump to section"                                                              | "ကဏ္ဍသို့ သွားရန်"                                           |
| `nav_bubble_top`  | "Top"                                                                            | "အပေါ်"                                                               |
| `nav_bubble_bottom` | "Bottom"                                                                       | "အောက်"                                                              |
...
The four Roboto Mono WOFF2 files are additionally **subsetted** (Latin, Latin Extended, Greek, punctuation, currency, arrows, math, box-drawing, block elements, geometric shapes, dingbats) by `scripts/subset_fonts.sh` — ~49 KB → ~30 KB each. `convert_fonts.py` skips existing WOFF2 files, so after replacing a Roboto Mono TTF run `subset_fonts.sh`, not `convert_fonts.py`. Thit_Sar_Shwe_Si is not subsetted (already Myanmar-only; subsetting saved <3%). `header.html` preloads `RobotoMono-Regular.woff2` on every page and `Thit_Sar_Shwe_Si.woff2` on Burmese pages.
...
Implemented in `assets/css/console.css` using `html:lang(my)` overrides. Note the two different codes: Hugo's language key is `mm` (URL prefix `/mm/`, `i18n/mm.yaml`, `.Site.Language.Lang` checks in templates), but the HTML `lang` attribute — set in `baseof.html` — is the BCP 47 tag `my`, which is what screen readers, hyphenation, and `hreflang` need. CSS therefore targets `html:lang(my)`, never `mm`:

| Area                                                          | Typeface in Burmese mode | Key selectors                                                                                              |
| ------------------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------- |
| Main content body (home/about/blood content)                  | Handwriting (`ArchitectsDaughter` + `Thit_Sar_Shwe_Si`; `[font: neat]` uses `Z01-Umoe002`) | `html:lang(my)` sets `--font-stack: var(--handwriting-font-stack)` |
...
Maintenance rule: treat EN as source-of-truth defaults and add the smallest possible `html:lang(my)` overrides. For new UI in Burmese mode, classify it first as **filled/overwritten content** (handwriting) or **printed/system output** (monospace), then scope selectors under the existing MM override block without changing global EN defaults.
...
The `navBubble*` keys are i18n strings rendered per-language. To make this work, `scripts.html` bundles `config.js` under a **per-language output name** — `js/config.js` for English, `js/config.mm.js` for Burmese — so the `{{ i18n }}` values baked into the bundle match the active language (see §6.1 for the load order).

| Key                  | Consumed by                                                  |
| -------------------- | ------------------------------------------------------------ |
| `unsplash`         | `unsplash.js`                                              |
| `lastfm`           | `lastFM.js`                                                |
| `cacheTTLMinutes`  | `cache.js`, `openalex.js`, `unsplash.js` |
| `researchTagLimit` | `research.js`                                              |
| `navBubbleLabel`   | `nav_bubble.js` (menu trigger label)                        |
| `navBubbleTop`     | `nav_bubble.js` (top row label)                             |
| `navBubbleBottom`  | `nav_bubble.js` (bottom row label)                          |
...
math: false      # true → loads self-hosted KaTeX CSS (<head>) and JS (</body>) for LaTeX rendering
mermaid: false   # true → loads mermaid-init.js (Mermaid ESM from jsDelivr) for ```mermaid fences
...
**Math rendering:** set `math: true` to enable KaTeX on that post. Inline math: `$...$`. Display (block) math: `$$...$$`. Requires the goldmark passthrough extension in `hugo.toml` (already configured) — without it, goldmark processes `_` and `\` inside delimiters and breaks the LaTeX. KaTeX 0.16.22 is **self-hosted** under `static/vendor/katex/0.16.22/` (`katex.min.css`, `katex.min.js`, `contrib/auto-render.min.js`, `fonts/*.woff2` only); `static/js/katex-init.js` runs `renderMathInElement` after the two deferred library scripts. To upgrade: download the npm tarball, copy those files into a new version directory, and change the three paths in `posts/single.html`.

**Bundle resources are not auto-published.** `content/posts/_index.md` cascades `build.publishResources: false` over `/posts/**` (and `content/blood-records/index.md` sets it directly), so a file dropped into a post bundle only reaches `public/` if a template or shortcode calls `.RelPermalink`/`.Permalink` on it (render hook, `fig`, `gallery`, `image:` front matter). Raw multi-megabyte originals therefore stay out of the deploy. Link PDFs and other downloadable files from `static/` instead.
...
| `education`      | Array of degrees: `degree`, `diploma`, `university`, `time`, optional `details`; transcript `diploma` values point to localized protected-document pages | `sidebar-content.html`                      |
...
### `data/protected_documents.yml`

Map keyed by the `protected_document` identifier used in page front matter. Each entry provides `document_id`, an opaque static `document_path` under `/general/documents/`, a public salt, and a SHA-256 `password_hash`. `layouts/_default/protected-document.html` validates the key at build time and exposes only the selected entry to `protected-document.js`.

This is a casual client-side privacy gate, not access control: the URL, salt, and verifier are shipped to the browser and can be bypassed or attacked offline. The PDFs remain public static files, including through repository history and caches. Their opaque current paths have exact Netlify `private, no-store` and `noindex, noarchive, nosnippet` headers; the former descriptive paths retain those headers and forcibly redirect to the gate pages. These measures reduce casual discovery but do not make the files private.
...
| `show_phone_number`      | Boolean; include the phone number in generated CV files (defaults to `true`)            |
...
Embedded as JSON in `theme-data.html` and read by `static/js/theme-init.js` — a classic (non-module) script loaded synchronously from the same partial, so it runs before `<body>` renders and eliminates flash of unstyled content (FOUC). It is external rather than inline so the CSP can omit `'unsafe-inline'`.
...
- title: "2026 Top Tracks"
  ...
  image: "playlist_2026.webp"  # filename in assets/images/playlists/ (processed by partials/playlist-cover.html)
```

Consumed by `panel-lastfm.html` to render annual playlist links. Each item's `year` value is displayed as a primary-color cover stamp (`.playlist-year`); CSS uses deterministic `nth-child()` offsets/rotations so stamps do not align uniformly across the strip.
...
│   ├── theme-data.html        themes.yml JSON blob + <script src="/js/theme-init.js"> (blocking, writes CSS vars, no FOUC)
...
└── .right-panels
    ...
    ├── panel-lastfm.html
    ├── panel-anilist.html
    ├── panel-simkl.html        (screen.sh — build-time Simkl fetch, static strip)
    ├── panel-unsplash.html
    └── panel-privacy.html
```
...
| ----------------- | --------------------------- | ------------------------------------------------------------------------------------- |
| `head-extra`    | Before `</head>`          | `posts/single.html` (self-hosted KaTeX CSS when `math: true`)                     |
| `scripts-extra` | Before `</body>`          | `404.html` (`not_found.js`), `posts/single.html` (KaTeX JS + `katex-init.js` when `math: true`; `mermaid-init.js` when `mermaid: true`), `_default/protected-document.html` (`protected-document.js`) |
...
**`_default/protected-document.html`** — Data-driven privacy gate for static documents. At 768px and wider it renders a CSS-only blurred placeholder and compact `VIEW` button; `VIEW` opens an `unlock.sh` terminal window over the placeholder. On mobile (<768px), the placeholder and `VIEW`/`EXIT` control are omitted and `unlock.sh` is shown directly. The gate uses `$ document-open --verify`, a real numeric-keyboard password field whose platform glyphs are visually replaced by a uniform spaced `*` mask, and CLI-style status lines. After successful SHA-256 verification, the script fetches the static PDF and assigns a page-lifetime `blob:` URL to the viewer: tablet/desktop gets an embedded native iframe plus `OPEN PDF`, while mobile gets a short `OPEN PDF` card for the browser's reliable top-level multi-page viewer. Reopening skips verification while the shared cache entry remains valid. Unlocks use the standard cache key `protected-document:<document_id>:unlocked` (stored under `cache:`), so they follow `cacheTTLMinutes`, work across tabs, and are removed by the privacy-panel flush. Each document has a separate key. On desktop, the protected-document main column stretches to the right-panel row height and keeps its footer at the bottom with at least 100px separation from the gate.
...
4. **Optional enhancers via `scripts-extra` / `head-extra`**:
   - `math: true` → self-hosted KaTeX CSS in `<head>`; KaTeX + auto-render + `katex-init.js` (all `defer`, in that order) before `</body>`
   - `mermaid: true` → `mermaid-init.js` module (imports Mermaid 11 from jsDelivr) replaces fenced `language-mermaid` code blocks with themed diagrams and re-renders on `theme-changed`
...
| --------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `header.html`             | `baseof.html`                                           | `<title>`, description, OG tags (incl. `og:locale` / `og:locale:alternate`, `og:image:alt`), Twitter Card (+ `twitter:image:alt`), canonical, robots, RSS link; `hreflang` alternates (`en`, `my`, `x-default` → English) on translated pages; `<link rel="preload">` for LCP avatar image + `RobotoMono-Regular.woff2` (+ `Thit_Sar_Shwe_Si.woff2` on `/mm/`); `preconnect` for OpenAlex + Last.fm (and GoatCounter in prod); `dns-prefetch` for AniList, Unsplash, simkl.in (screen.sh poster CDN), doi.org |
| `favicon.html`            | `baseof.html`                                           | `<link>` tags for all favicon sizes + webmanifest                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `theme-data.html`         | `baseof.html`                                           | Embeds `themes.yml` as a `type="application/json"` blob and loads `static/js/theme-init.js` synchronously (`data-ttl`, `data-lang` attributes) — writes CSS vars, `data-theme`, `data-mm-font`, and `<meta name="theme-color">` before render                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `panel-post-toc.html`     | `baseof.html` (post pages only)                         | Post table of contents panel (`toc.sh`); appears only when rendered content contains at least two section headings; includes 1-level / 2-level toggle whose logic lives in `static/js/post_toc.js` (module tag emitted by the partial)                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `panel-openalex.html`     | `baseof.html`                                           | Shell window for OpenAlex metrics panel                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `panel-research.html`     | `baseof.html`                                           | Shell window for research topics; emits `<script type="application/json" id="research-tags-data">{"tags":[…],"worksCount":N}</script>` (all tag occurrences + total works page count) which `research.js` parses                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `panel-github.html`       | `baseof.html`                                           | Shell window for GitHub projects (`github.sh`). **Fetches GitHub at build time**: for every repo in `data/github_repository.yml` it calls `resources.GetRemote` on `/repos/{owner}/{name}` and `/languages` (optional `HUGO_GITHUB_TOKEN` env → `Authorization: Bearer`), wrapped in `try` so a failure only `warnf`s. Bakes `{ fetchedAt, groups[].repos[].{name,label,data?} }` (data = `name`, `html_url`, `pushed_at`, `_langs`) into `<script type="application/json" id="github-projects-data">`; `github.js` renders it with **no client-side GitHub requests** unless data is missing or older than 7 days. Response cache `[caches.getresource] maxAge = "1h"`. Config: `groups[].owner`, `groups[].label`, `groups[].repos[].name`, `groups[].repos[].label`; prompt line uses `site.Params.promptName` |
| `panel-lastfm.html`       | `baseof.html`                                           | Shell window for Last.fm now-playing + playlists; playlist covers rendered through `playlist-cover.html`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `playlist-cover.html`     | `panel-lastfm.html`                                     | `resources.Get "images/playlists/<file>"` → `Fill` 64×64 and 128×128 WebP; `<img>` with `srcset="… 64w, … 128w" sizes="64px"`; `warnf` (no image) when the file is missing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `responsive-img.html`     | `render-image.html`, `fig.html`, `gallery.html`         | Shared `<img>` emitter for page-bundle rasters: WebP variants at the `widths` ladder (default 480/960/1600; gallery passes 240/480/960/1600), never upscales, source width appended as largest variant when smaller than the ladder top; `src`/`width`/`height` describe the largest; `loading="lazy" decoding="async"`, `sizes` from the caller                                                                                                                                                                                                                                                                                                                              |
| `image-sizes.html`        | the three image templates                               | Returns the `sizes` string for a context: `main` (post body), `figure` (60px side margins), `gallery` (4-column grid). Values derive from `.container` (95vw − 40px) and the `.terminal-layout` grid breakpoints — update if the column layout changes                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `panel-anilist.html`      | `baseof.html`                                           | `manga.sh` — one AniList strip `#last-read-manga` (`$ anilist manga --sort=updated`) filled by `anilist.js`; stamp = progress (`CH 123`). The former second strip `#last-watched-anime` (otaku.sh) is retired 2026-09 — anime moved to `screen.sh` — and kept as a Hugo comment block for revival |
| `panel-simkl.html`        | `baseof.html`                                           | `screen.sh` — **build-time** Simkl strip (`#last-watched`, `$ simkl list --status=recent`): `resources.GetRemote` on `/sync/all-items/movies`, `/shows` and `/anime` (bearer token from env), items with `last_watched_at` sorted desc, `first site.Params.screenLimit`; per item computes `href` (`simkl.com/{movies|tv|anime}/{id}/{slug}`), `img` (`https://simkl.in/posters/<poster>_c.webp`, 170×250), hover `label` (`Romaji (English) (Year) · ep 10/12`; English from one `/anime/{id}?extended=full` call per anime item) and `stamp` (Simkl `last_watched` progress: `S05E06` shows, `E10` anime, `S2026E818` year-seasons trimmed to `E818`; movies `FILM`), and emits them as `<script type="application/json" id="simkl-data">{fetchedAt, items}</script>` (or `{error:true}`) for `simkl.js`. Visitors never call Simkl. Missing env or any fetch error → `warnf` + error blob. Build-side responses cached 1h via `[caches.getresource]` |
...
| `footer.html`             | `baseof.html`                                           | Footer links (Hugo, Source, Me, CC BY license badge, language switcher); each item wrapped in `<span>` so `.footer` flex layout distributes them evenly (`justify-content: space-evenly`); CC badge uses two separate icons `cc.svg` + `by.svg` in `static/images/icons/`; `[Me]` click intercepted by `footer_roll.js` for the whoami animation; language switcher shows `[Bur]` (on EN pages) or `[Eng]` (on MM pages) as an inline text badge (`.lang-badge`, `var(--primary-color)`) — flows as plain text alongside the other footer links; if no translation exists for the current page the wrapping `<span>` gets `.is-unavailable` which adds `position: relative; display: inline-block` and draws a circle-and-slash via `::before`/`::after` pseudo-elements in `var(--error-color)`; its accessible name is an `.sr-only` child (not `aria-label`, which is prohibited on a plain span); the `[Me]` link's `aria-label` is `"Me — about the author"` so the visible text is part of the accessible name (WCAG 2.5.3) |
| `scripts.html`            | `baseof.html`                                           | `<script type="module">` tags for all JS modules; GoatCounter tag (prod only) carries `data-goatcounter-settings='{"allow_local": false}'` instead of an inline settings script; followed by `{{ block "scripts-extra" }}` for page-specific additions                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `works-filter-group.html` | `works.html`                                            | Renders one filter group (condition/datasource/method)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

**Removed partials:** the base-theme `opengraph.html` and `twitter_cards.html` were deleted 2026-09-08 (never invoked — OG and Twitter tags are handled entirely in `header.html`). `assets/css/animate-4.1.1.min.css` was removed at the same time (never linked; the `animateStyle` class on `.terminal-layout` has no matching rule).
...
| `gallery`           | `match="*.jpg"`, optional `page="/blood-records"` | responsive image grid | Page bundle resources (own page, or another bundle via `page=` — resolved with `site.GetPage`, falling back to the English site so `/mm/` pages can share a bundle; `errorf` if missing) | WebP `<img>` via `responsive-img.html` (240/480/960/1600 ladder, `gallery` sizes); link opens a 2000px WebP (or the WebP of the original if narrower) in a new tab — never the raw file; GIF/SVG skipped |
| `fig`               | `num`, `src`    | `<figure>` with caption          | Page bundle resources           | WebP `<img>` via `responsive-img.html` (480/960/1600 ladder, `figure` sizes); `alt=""` (caption is the accessible label); GIF/SVG fall back to a plain `<img>` |
...
| Image   | `render-image.html`   | Page bundle rasters: WebP `<img>` via `responsive-img.html` (480/960/1600 `srcset`, `main` sizes, `loading="lazy" decoding="async"`, explicit dimensions, alt/title preserved). GIF, SVG, external and absolute paths: plain `<img class="img-responsive">` |
...
All files in `static/js/`, loaded as `type="module"` in `partials/scripts.html` — with four exceptions that are loaded elsewhere: `theme-init.js` (classic blocking script in `<head>` via `theme-data.html`), `post_toc.js` (module tag emitted by `panel-post-toc.html`), `katex-init.js` and `mermaid-init.js` (`posts/single.html` `scripts-extra`). No inline `<script>` is allowed anywhere — the enforced CSP (§2.2) has no `'unsafe-inline'` for scripts; pass build-time values via `data-*` attributes or `type="application/json"` blobs. GoatCounter analytics script loads only in production (`{{ if not hugo.IsServer }}`). `scripts.html` loads `config.js` first — bundled under a per-language name (`js/config.js` for English, `js/config.mm.js` for Burmese) so its baked-in `{{ i18n }}` values match the active language — then `nav_bubble.js`, whose menu depends on `.category-header` elements being present in the DOM.

### 6.1 Module overview

| File                    | Imports      | DOM target                                                                                                  | External API                                       | Cache key                                                       | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------------------- | ------------ | ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cache.js`            | —           | —                                                                                                          | localStorage                                       | `cache:<key>`                                                   | Shared cache namespace for API panels. Exports `getCache(key)`, `setCache(key, data)`, `listCacheEntries()`, and `clearCache()`. TTL from `window.CONFIG.cacheTTLMinutes`; cache records carry `__cache: true` and legacy bare-key entries are migrated on read.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `cache-expires.js`    | `cache.js`  | `#cache-expire-line`, `#cache-flush-btns`                                                               | localStorage                                       | cache namespace + theme/font prefs                              | Shows soonest cache expiry, schedules one auto-reload when the earliest cache entry reaches TTL, and resets the cached session back to default presentation. Flush buttons clear cache plus theme/font preferences.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `nav_bubble.js`       | —           | `.category-header`, `#nav-bubble`, `#nav-bubble-menu`                                                 | `window.CONFIG`                                  | —                                                              | Floating section-navigation bubble. Self-gating: only engages when the page has 2+ `.category-header` (home, works, posts list, blood). Fixed `↑` top row → section rows (one per `.category-header`) → fixed `↓` bottom row, rendered as frosted-glass pills (`color-mix` + `backdrop-filter`). `role="menu"`/`menuitem`, `aria-expanded`/`aria-controls`/`aria-label`; opens on click, closes on Escape / outside click; opens moves focus to first row, jumps return focus to the bubble. Active section highlighted via `IntersectionObserver` (`rootMargin: 0 0 -70%`). Positioning JS-owned: mobile (<768px) bottom-right of viewport, desktop right edge aligned to `.main-content`. Smooth scroll disabled when `prefers-reduced-motion: reduce`; labels come from `window.CONFIG.navBubbleTop`/`navBubbleBottom`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `research.js`         | —           | `#research-cloud`, `#research-tags-data` (JSON blob)                                                    | —                                                 | —                                                              | Sorted `cli-table`: tag name (linked), 10-segment `█`/`░` block bar (% of all works, `MAX_BAR=10`), padded "X works" count; tooltip on whole row shows "X of Y works · click to search"; limit from `window.CONFIG.researchTagLimit`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `github.js`           | `cache.js` | `#github-projects`, `#github-projects-data` (JSON blob)                                                 | GitHub REST API (fallback only)                    | `github-projects` (whole baked payload), `github-<owner>-<name>` (live fetches) | Payload order: localStorage copy of the baked payload (unless the page's `fetchedAt` is newer) → baked payload (then `setCache`) — same lifecycle as simkl.js. Then per repo: baked `data` → localStorage cache → live fetch (only when baked data is missing or `fetchedAt` > `STALE_AFTER_MS` = 7 d) → placeholder row (name + link, empty bar/time). Panel shows the `$ api is not aping 🐒` error only when nothing at all has `pushed_at`. Tree table (owner → repos sorted by `pushed_at` desc); three columns: repo name, 10-segment `█` language bar (largest-remainder allocation, linguist colors; Okabe & Ito in colorblind mode), padded relative time; row tooltip shows repo name + language breakdown + "click to open repo"; bar click navigates to repo; placeholder `░░░░░░░░░░` for repos with no language data; re-renders on `theme-changed` event; error state: `$ api is not aping 🐒` |
...
| `protected-document.js` | `cache.js` | `[data-protected-document]`                                                                              | Web Crypto, Fetch, Blob URLs                       | `protected-document:<document_id>:unlocked`                    | Protected-document pages only (via `scripts-extra`); controls the tablet/desktop `VIEW`/`EXIT` gate and opens `unlock.sh` directly on mobile, hashes the configured salt plus trimmed input (Web Crypto with a local SHA-256 fallback), and only fetches the PDF after a match or valid cached unlock. It deduplicates the fetch, assigns a temporary `blob:` URL to the tablet/desktop iframe and all `OPEN PDF` actions, revokes it on `pagehide`, and guards async verification/loading against EXIT/Escape races. Unlock state is separate per document and follows the shared cache TTL/flush lifecycle. Client-side privacy UX only, not authentication. |
...
| `lastFM.js`           | `cache.js` | `#lasttrack`, `#topalbums`                                                                              | Last.fm API                                        | `lastfm-topalbums`                                            | Now-playing (no cache, 30s poll); top albums (cached) render album art with visible artist-name primary-color stamps (`.topalbum-artist`); full artist/album/play-count text remains in image `alt`/`title`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `simkl.js`            | `cache.js` | `#last-watched`, `#simkl-data`                                                                            | — (build-time blob only)                           | `simkl-recent-<n>`                                            | Renders the `screen.sh` strip from the JSON blob baked by `panel-simkl.html`; order: localStorage copy (unless the blob's `fetchedAt` is newer) → blob (then `setCache`). Markup `<a><img><span class="screen-stamp">` mirrors anilist.js; `{error:true}` → `$ api is not aping 🐒`, empty → `$ no recent activity`. No network requests |
| `anilist.js`          | `cache.js` | `#last-read-manga`                                                                                        | AniList GraphQL                                    | `anilist-<user>-recent-<limit>-manga`                        | One GraphQL query (`manga` `Page.mediaList`, `status_not: PLANNING`, sort UPDATED_TIME_DESC, `perPage = ANILIST_LIMIT`) for user "finer" — most recently updated entries regardless of reading/finished/dropped/paused; `$ no recent activity` when empty; stamp (`.otaku-stamp`) = progress `CH n`; hover/alt = `Romaji (English) · reading · ch 7/24` — English only when it differs from romaji. The `anime` alias / `#last-watched-anime` strip (`E7` stamps) is retired 2026-09 (screen.sh covers anime) and kept commented out |
| `unsplash.js`         | `cache.js` | `#latestimage`                                                                                            | Unsplash API                                       | `unsplash-photos`                                             | Latest 10 photos from account "finerbrighterlighter"; each thumbnail rendered as a link to the Unsplash photo page; publish year displayed as a primary-color cover stamp (`.photo-date`) with deterministic `nth-child()` offsets/rotations                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `anilist.js`       | `ANILIST_LIMIT`           | `10`                     | Covers shown in the manga strip                                                                                   |
| `nav_bubble.js`    | `GLYPH_CLOSED`            | `"≡"`                   | Glyph on the bubble button when the menu is closed |
| `nav_bubble.js`    | `GLYPH_OPEN`              | `"×"`                   | Glyph on the bubble button while the menu is open |
| `nav_bubble.js`    | mobile breakpoint         | `767` px                | Below this width the bubble sits at the bottom-right of the viewport; ≥768px it aligns to the `.main-content` right edge |
| `nav_bubble.js`    | `debounce`                | `150` ms                | Resize debounce for repositioning |
| `nav_bubble.js`    | close delay               | `170` ms                | Delay before the menu element gets `hidden` after closing (lets the fade finish) |
...
| `github.js`        | `STALE_AFTER_MS`          | 7 days                   | Age of the build-time GitHub payload beyond which one live refresh is attempted per repo (cached via `cache.js`) |
...
- `setCache(key, data)` writes to `localStorage["cache:<key>"]`.
- Cache records are shaped like `{"__cache": true, "data": <payload>, "ts": <unix-ms>}`.
...
- Freshness check is `Date.now() - ts <= TTL`; if the difference is greater than TTL, the cached value is ignored and fresh data is fetched.
- Expired entries are deleted lazily on read.
- Legacy cache entries stored under bare keys are migrated into the `cache:` namespace on first successful read.
- Helper exports include `listCacheEntries()`, `clearCache()`, and `clearSiteStorage()` for session reset behavior.
...
| `github-projects`                    | `github.js`        | Whole build-time `github.sh` payload (`fetchedAt` + groups); refreshed when a page ships a newer `fetchedAt` |
| `simkl-recent-<n>`                   | `simkl.js`         | Build-time `screen.sh` blob (`fetchedAt` + `n` ready-to-render items); refreshed when a page ships a newer `fetchedAt` |
| `anilist-<username>-recent-<limit>-<strips>` | `anilist.js` | Manga recently-updated list response payload (`<strips>` = `manga`; was `manga+anime` for otaku.sh)                    |
| `unsplash-<username>-<limit>`        | `unsplash.js`      | Reduced photo metadata used for rendering                           |

**Privacy panel integration (`cache-expires.js`):**

- The `[expires]` line scans cache-managed entries only and shows the soonest remaining TTL.
- A one-shot timer is scheduled for the earliest cache expiry in an open tab.
- When that timer fires, cached API data and visual preferences are cleared and the page reloads after `FLUSH_RELOAD_DELAY` (400ms), creating a fresh default session.
...
`static/js/theme-init.js` (the blocking script loaded from `partials/theme-data.html`) runs before body render on every page load:
...
|---|---|---|
| API responses (OpenAlex, Last.fm, AniList, Unsplash, BibTeX) and the build-time `screen.sh` / `github.sh` payloads (`simkl.js`, `github.js`) | `cache.js` via `setCache()` | Yes — triggers full reset |
...
**Initialization:** `theme-init.js` (blocking, loaded from `theme-data.html`) writes CSS variables to `:root` and the initial `theme-color` meta before `<body>` renders. `theme.js` then wires up the UI controls and syncs the dropdown selection to the current state.
...
### `subset_fonts.sh` — Roboto Mono subsetting

`bash scripts/subset_fonts.sh` (needs `pyftsubset` from fonttools on PATH) regenerates the four `RobotoMono-*.woff2` files from their TTFs with a fixed Unicode allowlist (Latin, Latin Extended, Greek, general punctuation, currency, letterlike, arrows, math operators, box drawing, block elements, geometric shapes, dingbats, PUA). ~49 KB → ~30 KB per face. Glyphs outside the list fall back to the next font in the stack. Because `convert_fonts.py` skips existing WOFF2 files, run this script — not `convert_fonts.py` — after replacing a Roboto Mono TTF. Do not subset Thit_Sar_Shwe_Si (already Myanmar-only).
...
| ---------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `header.html`        | Every page                      | `<meta name="description">` (citation-style string on journal works, abstract on non-journal works, summary on posts, site description elsewhere); `og:title`, `og:description`, `og:type` (`website` home / `article` others), `og:image` + `og:image:alt`, `og:logo` (apple-touch-icon.png), `og:site_name`, `og:locale` (`en_US` / `my_MM`) + `og:locale:alternate` for translations; Twitter Card tags (+ `twitter:image:alt`); `<link rel="canonical">` (guarded — only emitted when `.Permalink` is non-empty, i.e. not on conference/report suppressed pages); on translated pages `<link rel="alternate" hreflang="en|my">` for each translation plus `hreflang="x-default"` pointing at the English page (Hugo key `mm` is mapped to BCP 47 `my`) |
| `baseof.html`        | Every page                      | `<html lang>` (`en`, or `my` for the `mm` site); `<meta name="theme-color">` (placeholder `#2d2a2e`, overwritten by `theme-init.js` / `theme.js` with the active palette background) |
...
Nav bubble (see §6.1) refines this with two extra targets: `#nav-bubble:focus-visible` uses a 2px solid `--primary-color` outline at **3px** offset (the bubble is round and needs clearance), and `#nav-bubble-menu .nav-bubble-row:focus-visible` uses the same color at 70% opacity via `color-mix()` with a **−2px** inset outline (rows are flush pills).
...
- `[Me]` link: `aria-label="Me — about the author"` — the visible text is included in the accessible name (WCAG 2.5.3 label-in-name).
- Unavailable language badge: a plain `<span>` may not carry `aria-label`, so the description lives in an `.sr-only` child span; `title` kept for hover.

### Heading order

Homepage experience/project titles are `<h3>` under the `<h2 class="category-header">` sections (were `<h4>`, which skipped a level). Headings inside post bodies are author-controlled; a post that opens with `###` will still trip Lighthouse's heading-order check.

### pa11y audit results

Full audit results are in `docs/pa11y/{theme}/` (CSV per page) + `docs/pa11y/README.md` summary. Spot check 2026-09-08 (pa11y 9.1.1, Duck default, after the CSP / footer / heading changes): `/`, `/works/`, a long post, and `/mm/` all report 0 issues. Re-run the full matrix with:
...
The runner pre-seeds the selected mode and palette in `localStorage` via a shared Puppeteer browser before each page test, so `theme-init.js` applies the correct theme on load. Hugo dev server must be running on `localhost:1313`.
