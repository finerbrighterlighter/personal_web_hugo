...
| `HUGO_SIMKL_CLIENT_ID` | `simkl.js` (via `config.js`) | Simkl app client id (simkl.com/settings/developer). Sent as `?client_id=` on every Simkl call |
| `HUGO_SIMKL_TOKEN`     | `simkl.js` (via `config.js`) | Long-lived Simkl PIN-flow bearer token for the site owner's library. Baked into `config.js` and therefore visible in page source — accepted, like the Last.fm key; unlike it, this token can also **write** to the Simkl account (Simkl has no read-only scope). Without both vars the `screen.sh` strip renders `$ api is not aping 🐒` |
...
| `lastfm`           | `lastFM.js`                                                |
| `simklClientId`    | `simkl.js`                                                 |
| `simklToken`       | `simkl.js`                                                 |
| `screenLimit`      | `simkl.js`                                                 |
| `cacheTTLMinutes`  | `cache.js`, `openalex.js`, `simkl.js`, `unsplash.js` |
...
| `panel-simkl.html`        | `baseof.html`                                           | `screen.sh` — shell window (`$ simkl list --status=recent` linking to the Simkl profile) with the empty `#last-watched` `.album-strip`; filled client-side by `simkl.js` |
...
| `simkl.js`            | `cache.js` | `#last-watched`                                                                                           | Simkl API (browser, bearer token from `config.js`) | `simkl-recent-<n>`                                            | `GET /sync/all-items?date_from=<14 d ago>` (few KB); if fewer than `screenLimit` watched titles, `GET /sync/all-items` (full library, brotli). Three buckets flattened, sorted by `last_watched_at` desc, top `n`; `GET /anime/{id}?extended=full` per anime item for `en_title`. Renders `<a href=simkl.com/{movies|tv|anime}/{id}/{slug}><img simkl.in/posters/<poster>_c.webp 64×64><span class="screen-stamp">` — stamp = `last_watched` progress (`S05E06`, `E10`, `S2026E818`→`E818`; movies `FILM`), hover `Title (English) (Year) · ep 30/36`; `$ api is not aping 🐒` on error / missing credentials, `$ no recent activity` when empty |
...
| `simkl-recent-<n>`                   | `simkl.js`         | Curated `screen.sh` list (`fetchedAt` + `n` ready-to-render items) from the browser-side Simkl fetch |
...
|---|---|---|
| API responses (OpenAlex, Last.fm, AniList, Simkl, Unsplash, BibTeX) and the build-time `github.sh` payload (`github.js`) | `cache.js` via `setCache()` | Yes — triggers full reset |
