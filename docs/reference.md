...
| `work-bibtex.js` | `cache.js` | `article[data-doi]`, `#bibtex-btn` | doi.org (content negotiation) | `bibtex-<doi>` | Work pages only; CITE button opens `fetch.sh` terminal panel; format selector (bibtex/nlm/apa/ama); Copy; BIB download; RIS download; collapse via `collapse_windows.js`, close via EXIT. BibTeX display: prettified (one field per line, aligned keys, preferred field order), syntax-highlighted (keys in primary, punctuation in secondary), Courier New at `--font-sm`; Copy and BIB download use raw doi.org response. NLM: all authors listed. AMA: ≤6 authors listed in full; >6 shows first 3 + et al. |
