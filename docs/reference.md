...
Consumed by `panel-lastfm.html` to render annual playlist links. Each item's `year` value is displayed as a red cover stamp (`.playlist-year`); CSS uses deterministic `nth-child()` offsets/rotations so stamps do not align uniformly across the strip.
...
| `lastFM.js`           | `cache.js` | `#lasttrack`, `#topalbums`                                                                              | Last.fm API                                        | `lastfm-topalbums`                                            | Now-playing (no cache, 30s poll); top albums (cached) render album art with visible artist-name red stamps (`.topalbum-artist`); full artist/album/play-count text remains in image `alt`/`title`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
