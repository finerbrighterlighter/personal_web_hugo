...
| `HUGO_TRAKT_KEY`    | —              | Trakt API client ID — **disabled 2026-08** (Trakt requires VIP for API apps); retained for future re-enable |
| `HUGO_TMDB_KEY`     | —              | TMDB API key (for poster images) — **disabled 2026-08** alongside `trakt` |
...
| `trakt`            | — (disabled 2026-08; retained)                              |
| `tmdb`             | — (disabled 2026-08; retained)                              |
| `lastfm`           | `lastFM.js`                                                |
...
└── .right-panels
    ...
    ├── panel-lastfm.html
    ├── panel-anilist.html
    ├── panel-trakt.html        (disabled 2026-08 — Trakt requires VIP for API apps; partial retained, not rendered)
    ├── panel-unsplash.html
    └── panel-privacy.html
```
...
| `panel-anilist.html`      | `baseof.html`                                           | `otaku.sh` — two AniList strips: `#last-read-manga` (`$ anilist manga --sort=updated`) and `#last-watched-anime` (`$ anilist anime --sort=updated`), separated by `.panel-terminal-gap`; both filled by one request from `anilist.js` |
| `panel-trakt.html`        | — (disabled 2026-08)                                  | Shell window for Trakt watch history — commented out in `baseof.html`; Trakt requires VIP for API apps. Retained for future re-enable |
...
| `anilist.js`          | `cache.js` | `#last-read-manga`, `#last-watched-anime`                                                                 | AniList GraphQL                                    | `anilist-<user>-recent-<limit>`                              | One aliased GraphQL query (`manga` + `anime` `Page.mediaList`, `status_not: PLANNING`, sort UPDATED_TIME_DESC, `perPage = ANILIST_LIMIT`) for user "finer" — most recently updated entries regardless of watching/reading/finished/dropped/paused; hover title appends the status label; `$ no recent activity` when a list is empty; renders a list-status stamp (`.otaku-stamp`: WATCHING/READING, REWATCH/REREAD, DONE, DROPPED, PAUSED — ≤8 chars for the 52px stamp); hover/alt title = `Romaji (English) · ep 7/24` — English only when it differs from romaji, no status (on the stamp), no origin |
| `trakt.js`            | `cache.js` | `#last-watched`                                                                                           | Trakt + TMDB APIs                                  | `trakt-watched`                                               | **Disabled 2026-08** — Trakt requires VIP for API apps; `trakt.js` no longer loaded in `scripts.html`, `panel-trakt.html` not rendered in `baseof.html`, `api.trakt.tv` dns-prefetch removed from `header.html`. File retained for future re-enable |
...
| `anilist.js`       | `ANILIST_LIMIT`           | `10`                     | Covers shown per strip (manga and anime)                                                                                   |
| `trakt.js`         | `TRAKT_USER`              | `"hteza"`                | Trakt username — **disabled 2026-08**, retained for future re-enable |
| `trakt.js`         | `TRAKT_HISTORY_FETCH`     | `100`                    | Items fetched from API (must be ≥ TRAKT_DISPLAY_LIMIT) — **disabled 2026-08** |
| `trakt.js`         | `TRAKT_DISPLAY_LIMIT`     | `10`                     | Items shown in panel — **disabled 2026-08** |
...
| `anilist-<username>-recent-<limit>` | `anilist.js`       | Combined manga + anime recently-updated list response payload                                         |
...
| `panel-anilist.html`  | "Anime and manga"      |
| `panel-trakt.html`    | "Screen" — **disabled 2026-08** (Trakt requires VIP for API apps; panel not rendered) |
