...
| `panel-simkl.html`        | `baseof.html`                                           | `screen.sh` — **build-time** Simkl strip (`#last-watched`, `$ simkl list --status=recent`): `resources.GetRemote` on `/sync/all-items/movies`, `/shows` and `/anime` (bearer token from env), items with `last_watched_at` sorted desc, `first site.Params.screenLimit`; stamp `.screen-stamp` = Simkl `last_watched` progress (`S05E06` shows, `E10` anime; `S2026E818` year-seasons trimmed to `E818` to fit 52px; movies `FILM`); one `/anime/{id}?extended=full` detail call per anime item for `en_title` → hover `Romaji (English) (Year) · ep 10/12`; poster `https://simkl.in/posters/<poster>_c.webp` (170×250) in the 64×64 strip; links to `simkl.com/{movies|tv|anime}/{id}/{slug}`. Static HTML, no JS, no localStorage. Missing env or any fetch error → `warnf` + `.api-error` line. Cached 1h via `[caches.getresource]`, so new watches appear on the next Netlify build |
